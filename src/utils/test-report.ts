...
export const parseReportTypeData = (reportData: {
    ...
    // * CCODE_TYPE
    targetKey = keys.filter(e => e.includes('配方'))
    if (targetKey.length > 0) {
        const data = reportData[targetKey[0]]
        console.log('配方 data:', data)
        const items = data.filter(e => e.log && e.log.includes('CCODE'))
        if (items.length > 0) {
            const mResult = items[0].log.match(/<([A-Z]\d?) \[\d+\] .*?> \*CCODE/)
            if (mResult) {
                console.log('CCODDE TYPE:', mResult[1])
                result.cCodeType = mResult[1]
            }
        }
    }
    return result
